...
const vaRates = {
  ...
  70: {
    ...
    withOneChild: 1931.45, // Note: Usually calculated as Alone + child rate. Validating manually: 1808.45 + (2084.98-1961.45=123.53) = 1931.98? Search said 1931.45.
    addPerChild: 76.0,
  },
  80: {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("vaRating")
    .addEventListener("change", updateVADependentFields);
  document
    .getElementById("hasSpouse")
    .addEventListener("change", updateVADependentFields);
  document
    .getElementById("hasDependentParent")
    .addEventListener("change", updateVADependentFields);
...
function updateVADependentFields() {
  ...
  const hasDependentParentCheckbox =
    document.getElementById("hasDependentParent");
  ...
  if (hasDependentParentCheckbox.disabled)
    hasDependentParentCheckbox.checked = false;
}
...
function calculateSalary() {
  ...
  setTimeout(() => {
    try {
      ...
      const hasDependentParent =
        vaRating === "100" &&
        document.getElementById("hasDependentParent").checked;

      let desiredIncome =
        parseFloat(document.getElementById("desiredIncome").value) || 0;
      ...
      const vaCompensation = calculateVACompensation(
        vaRating,
        dependents,
        hasSpouse,
        hasDependentParent
      ...
      const result = calculateRequiredSalary(
        desiredIncome,
        vaCompensation,
        stateCode,
        filingStatus
...
function displayResults(result, vaMonthlyCompensation) {
  ...
  totalTakeHomeElement.textContent = formatCurrency(
    (result.netSalary + result.vaCompensation) / 12,
    2
  );
